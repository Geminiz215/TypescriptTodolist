import { useEffect, useState } from "react";
import NewLandingLayout from "./landingLayout";
import axios from "axios";
import clsx from "clsx";
import AddContent from "./addContent";
import { AiFillDelete } from "react-icons/ai";

type dataInput = {
  _id: string;
  name: string;
  description: string;
  todo: string;
  status: string;
  startDate?: string;
  endDate?: string;
};

const Tabel = () => {
  const [data, setData] = useState<dataInput[]>([]);
  const [loading, setLoading] = useState(true);
  const [refresh, setRefresh] = useState(true);

  const handleDelete = async (key: string) => {
    try {
      console.log(key, "ini di fe");
      const response = await axios.delete(`/api/hello?_id=${key}`);
      setRefresh(true);
      console.log(refresh, "ini di fe");
    } catch (error: any) {
      alert("Error making POST request: " + error.message);
    }
  };

  const formatDate = (time: string) => {
    let date = new Date(time);
    const formattedDate = date.toLocaleDateString("id-ID", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    return formattedDate;
  };

  const handleChange = (value: string, key: string) => {
    const apiUrl = "/api/hello";
    axios
      .put(apiUrl, { status: value, _id: key })
      .then(() => {
        setRefresh(true);
      })
      .catch((error) => {
        alert("Error making POST request: " + error.message);
      });
  };

  const receiveDataFromChild = (data: boolean) => {
    setRefresh(data);
  };

  useEffect(() => {
    const apiUrl = "/api/hello";
    axios
      .get(apiUrl)
      .then((response) => {
        setData(response.data.item);
        setLoading(false);
      })
      .catch((error) => {
        console.error("Error fetching data:", error);
        setLoading(false);
      });
    setRefresh(false);
  }, [refresh]);

  return (
    <NewLandingLayout className="pt-24">
      <h1 className="text-center my-2 text-2xl">Test Front end</h1>
      <div
        className={clsx(
          "fixed top-0 left-0 right-0 bottom-0 transition-all duration-500 bg-gray-500 bg-opacity-50",
          loading ? "opacity-100 " : "opacity-0 pointer-events-none"
        )}
      >
        <div className="w-screen h-screen bg-white flex items-center justify-center">
          <div role="status">
            <svg
              aria-hidden="true"
              className="inline w-10 h-10 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="currentFill"
              />
            </svg>
            <span className="sr-only">Loading...</span>
          </div>
        </div>
      </div>
      <table className="table-fixed border-separate  border border-spacing-2 border-slate-500 min-w-fit my-5 ">
        <caption className="caption-bottom">Table : Todo list</caption>
        <thead>
          <tr className="odd:bg-white even:bg-slate-50">
            <th className="border-b-2 border-black ">Name</th>
            <th className="border-b-2 border-black">Todo</th>
            <th className="border-b-2 border-black">Description</th>
            <th className="border-b-2 border-black">startDate</th>
            <th className="border-b-2 border-black">endDate</th>
            <th className="border-b-2 border-black">Status</th>
            <th className="border-b-2 border-black">Delete</th>
          </tr>
        </thead>
        <tbody>
          {data.map((item: dataInput) => (
            <tr key={item._id} className="hover:bg-gray-100">
              <td className="text-sm hover:border-b-2">{item.name}</td>
              <td className="text-sm hover:border-b-2">{item.todo}</td>
              <td className="text-sm hover:border-b-2">{item.description}</td>
              {item.startDate === null ? (
                <td></td>
              ) : (
                <td className="text-sm hover:border-b-2">
                  {item.startDate ? formatDate(item.startDate) : " "}
                </td>
              )}
              {item.endDate === null ? (
                <td></td>
              ) : (
                <td className="text-sm hover:border-b-2">
                  {item.endDate ? formatDate(item.endDate) : " "}
                </td>
              )}
              <td>
                <select
                  id="cars"
                  name="cars"
                  className="cursor-pointer"
                  value={item.status}
                  onChange={(e: any) => {
                    handleChange(e.target.value, item._id);
                  }}
                >
                  <option value="Done">Done</option>
                  <option value="OnProgress">OnProgress</option>
                  <option value="NotStarted">NotStarted</option>
                </select>
              </td>
              <td
                className="justify-center items-centern cursor-pointer pl-3"
                onClick={() => {
                  handleDelete(item._id);
                }}
              >
                <AiFillDelete size={20} />
              </td>
            </tr>
          ))}
        </tbody>
      </table>
      <AddContent sendDataToParent={receiveDataFromChild} />
    </NewLandingLayout>
  );
};

export default Tabel;
